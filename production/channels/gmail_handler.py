...
class GmailHandler:
    def __init__(self, credentials_path: str = None):
        ...
        try:
            if os.path.exists(self.credentials_path):
                ...
                print(f"Warning: Gmail credentials not found at {self.credentials_path}. Mode: MOCK")
                self.service = None
    ...
    async def process_notification(self, pubsub_message: dict) -> list:
        ...
        if not self.service:
            # Return a mock message for testing if service is not active
            return [{
                'channel': 'email',
                'channel_message_id': 'mock-msg-id',
    ...
    async def send_reply(self, to_email: str, subject: str, body: str, thread_id: str = None) -> dict:
        """Send email reply."""
        if not self.service:
            print(f"Mock Send Email to {to_email}: {body[:50]}...")
